...
Version 2.0 of `terminal-widgets` introduces a major refactor to clean up the API, improve typing, and centralize state management. If you have created custom widgets in `~/.config/twidgets/py_widgets/`, you will need to update them to work with the new release.

## 1.1 Abstract of Changes

- **Widgets locations**: All widgets (including built-ins) are now located at `~/.config/twidgets/py_widgets/`. Run `twidgets init` again  if you want to add them to your layout.
- **`WidgetContainer`**: Replaces the separate `UIState` and `BaseConfig` arguments.
